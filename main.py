...
def comp_settings(x, y, **conf):
	if 'settings' in config['pages'][config['active_page']]['sprites']:
		config['pages'][config['active_page']]['sprites']['settings']._delete()
		del(config['pages'][config['active_page']]['sprites']['settings'])

	print(config['virtual_machines'][conf['id']])
...
class generic_sprite(pyglet.sprite.Sprite):
	def __init__(self, texture=None, width=None, height=None, color="#C2C2C2", alpha=int(0), x=None, y=None, moveable=False, clickable=True, batch=None, group=None, bind=None, bparams={}, rparams={}, rbind=None, dbind=None, txtmode=None, name=None, linkable=False):
...
class computer(generic_sprite):
	def __init__(self, x=0, y=0, **kwargs):
		...
		generic_sprite.__init__(self, group=2, x=x, y=y, dbind=self.doubleClick, **kwargs)
		self.text = 'Computer'
		self.started = False
		self.tap = None
...
class menu(generic_sprite):
	def __init__(self, menu_options, x=0, y=0, **kwargs):
		...
		self.off = gen_solid_image(30, 30, color='#333333')
		generic_sprite.__init__(self, group=0, texture=self.off, x=x, y=y, txtmode='center', **kwargs)
		self.text = 'Right Click Menu'

config['pages'][config['active_page']]['sprites']['comp1'] = computer(x=0, y=config['resolution']['height']-30, bind=add_computer, moveable=False, txtmode='left')
config['pages'][config['active_page']]['sprites']['router1'] = router(x=0, y=config['resolution']['height']-60, bind=add_router, moveable=False, txtmode='left')
config['pages'][config['active_page']]['sprites']['switch1'] = switch(x=0, y=config['resolution']['height']-90, bind=add_switch, moveable=False, txtmode='left')
config['pages'][config['active_page']]['sprites']['linkButton'] = linker(x=0, y=config['resolution']['height']-120, moveable=False)
...
for root, folders, files in walk('./'):
	for filename in files:
		...
		config['pages'][config['active_page']]['sprites']['comp_'+name] = computer(texture=conf['icon'], x=x, y=config['resolution']['height']-30, bind=copy_template, bparams=conf, moveable=False, linkable=False, rbind=comp_settings, rparams={'name' : 'comp_'+name})
		...
		config['virtual_machines'][config['pages'][config['active_page']]['sprites']['comp_'+name].name] = conf
		x += 31
...
class line(generic_shape):
	def __init__(self, batch, sx, sy, ex, ey, color=(255,255,255,255)):
		generic_shape.__init__(self, batch, 2, GL_LINES, None, ("v2f", (sx, sy, ex, ey)), ('c4B', color*2))
